



# loop-unroll report

## Optimization by program

### % faster
  
![loop-unroll-by-program](./loop-unroll-by-program.png)
#### Optimization by program (risc0, % faster)
  
![loop-unroll-risc0-by-program](./loop-unroll-risc0-by-program.png)
#### Optimization by program (sp1, % faster)
  
![loop-unroll-sp1-by-program](./loop-unroll-sp1-by-program.png)
### Speedup
  
![loop-unroll-by-programspeedup](./loop-unroll-by-programspeedup.png)
#### Optimization by program (risc0, Speedup)
  
![loop-unroll-risc0-by-programspeedup](./loop-unroll-risc0-by-programspeedup.png)
#### Optimization by program (sp1, Speedup)
  
![loop-unroll-sp1-by-programspeedup](./loop-unroll-sp1-by-programspeedup.png)