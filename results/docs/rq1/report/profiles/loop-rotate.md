



# loop-rotate report

## Optimization by program

### % faster
  
![loop-rotate-by-program](./loop-rotate-by-program.png)
#### Optimization by program (risc0, % faster)
  
![loop-rotate-risc0-by-program](./loop-rotate-risc0-by-program.png)
#### Optimization by program (sp1, % faster)
  
![loop-rotate-sp1-by-program](./loop-rotate-sp1-by-program.png)
### Speedup
  
![loop-rotate-by-programspeedup](./loop-rotate-by-programspeedup.png)
#### Optimization by program (risc0, Speedup)
  
![loop-rotate-risc0-by-programspeedup](./loop-rotate-risc0-by-programspeedup.png)
#### Optimization by program (sp1, Speedup)
  
![loop-rotate-sp1-by-programspeedup](./loop-rotate-sp1-by-programspeedup.png)