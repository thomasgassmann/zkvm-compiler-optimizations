



# strip-dead-debug-info report

## Optimization by program

### % faster
  
![strip-dead-debug-info-by-program](./strip-dead-debug-info-by-program.png)
#### Optimization by program (risc0, % faster)
  
![strip-dead-debug-info-risc0-by-program](./strip-dead-debug-info-risc0-by-program.png)
#### Optimization by program (sp1, % faster)
  
![strip-dead-debug-info-sp1-by-program](./strip-dead-debug-info-sp1-by-program.png)
### Speedup
  
![strip-dead-debug-info-by-programspeedup](./strip-dead-debug-info-by-programspeedup.png)
#### Optimization by program (risc0, Speedup)
  
![strip-dead-debug-info-risc0-by-programspeedup](./strip-dead-debug-info-risc0-by-programspeedup.png)
#### Optimization by program (sp1, Speedup)
  
![strip-dead-debug-info-sp1-by-programspeedup](./strip-dead-debug-info-sp1-by-programspeedup.png)