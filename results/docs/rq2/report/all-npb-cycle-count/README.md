...
- Best profile: Profile(profile_name='genetic', rustflags='-C opt-level=3 -C lto=fat -C embed-bitcode -C codegen-units=1 -Cllvm-args=-inline-threshold=4328 -Cllvm-args=-jump-threading-implication-search-threshold=7 -Cllvm-args=-licm-max-num-uses-traversed=1 -Cllvm-args=-licm-versioning-max-depth-threshold=1 -Cllvm-args=-loop-distribute-scev-check-threshold=37 -Cllvm-args=-max-dependences=23 -Cllvm-args=-max-nested-scalar-reduction-interleave=2 -Cllvm-args=-max-speculation-depth=1 -Cllvm-args=-memdep-block-scan-limit=276', cflags='-O3 -mllvm -inline-threshold=4328 -mllvm -jump-threading-implication-search-threshold=7 -mllvm -licm-max-num-uses-traversed=1 -mllvm -licm-versioning-max-depth-threshold=1 -mllvm -loop-distribute-scev-check-threshold=37 -mllvm -max-dependences=23 -mllvm -max-nested-scalar-reduction-interleave=2 -mllvm -max-speculation-depth=1 -mllvm -memdep-block-scan-limit=276', passes=['function(lower-switch,partially-inline-libcalls,loop-unroll,sccp,dse,reassociate,mem2reg,separate-const-offset-from-gep,loop-data-prefetch,speculative-execution),module(inline),function(gvn),module(function-attrs,attributor),function(newgvn,loop-mssa(licm),irce,newgvn,loop(loop-reduce),loop-data-prefetch)'], prepopulate_passes=True, lower_atomic_before=False)
- Metric: 376764825
- Mode: depth-20
