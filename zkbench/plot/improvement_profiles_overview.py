...
def plot_improvement_profiles_overview(
    ...
    counts = {
        zkvm: {m: {p: np.zeros(4, dtype=int) for p in profiles} for m in metrics}
        for zkvm in zkvms
    }
    ...
    for metric in metrics:
        ordering = list(
            sorted(
                profiles,
                key=lambda p: sum(counts[zkvm][metric][p][0] for zkvm in zkvms),
                reverse=True,
            )
        )
        if top_n:
            ...
            profiles_per_metric[metric] = sorted(
