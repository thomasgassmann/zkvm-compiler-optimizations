...
pub fn exec(input: Vec<String>, range: std::ops::Range<usize>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let leaves_to_prove = leaves.get(range.clone()).ok_or("can't get leaves to prove")?;
