...
void run_epd_testsuite(gamestate_t *g, state_t *s, const char *testname) {
    ...
    while (*current != '\0') {
        // Read a line into readbuff
        int i = 0;
        while (*current != '\n' && *current != '\0' && i < STR_BUFF - 1) {
            readbuff[i++] = *current++;
        }
        readbuff[i] = '\0';
        ...
        // Read the next line for thinkdepth
        i = 0;
        while (*current != '\n' && *current != '\0' && i < STR_BUFF - 1) {
            readbuff[i++] = *current++;
        }
        ...
        thinkdepth = 0;
        for (int j = 0; readbuff[j] != '\0'; j++) {
            thinkdepth = thinkdepth * 10 + (readbuff[j] - '0');
        }

        printf("Analyzing %d plies...\n", thinkdepth);
        display_board(s, 1);
