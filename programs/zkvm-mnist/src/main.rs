...
impl NeuralNetwork {
    ...
    fn test(&self, data: &Vec<(Vec<f64>, Vec<f64>)>) {
        let mut correct = 0;
        ...
        println!("Accuracy: {}, correct: {}/{}", correct as f64 / data.len() as f64, correct, data.len());
    }

    fn train(&mut self, data: &Vec<(Vec<f64>, Vec<f64>)>, test: &Vec<(Vec<f64>, Vec<f64>)>, epochs: usize, lr: f64) {
        for i in 0..epochs {
            println!("Epoch {}", i);
            for (input, target) in data.iter() {
